...
const TEST_ENV_KEYS = [
  ...
  "GHOLA_CARRY_MAX_INDEX_PRICE_DIVERGENCE_BPS",
  "GHOLA_CARRY_MAX_MARK_PRICE_DIVERGENCE_BPS",
  ...
  "GHOLA_CARRY_SHADOW_OBSERVER_ENABLED",
  "GHOLA_CARRY_SHADOW_OBSERVER_INITIAL_DELAY_MS",
  "GHOLA_CARRY_SHADOW_OBSERVER_INTERVAL_MS",
  "GHOLA_CARRY_SHADOW_OBSERVER_STALL_MS",
  "GHOLA_CARRY_SHADOW_OBSERVER_ASSETS",
  "GHOLA_CARRY_SHADOW_QUALIFICATION_SAMPLES",
  "GHOLA_CARRY_SHADOW_QUALIFICATION_MAX_AGE_MS",
  ...
  "PRIVATE_AGENT_CARRY_SHADOW_OBSERVER_STALL_MS",
...
describe("private-agent Phala provisioning", () => {
  it("builds a no-plaintext worker compose with dstack quote binding", () => {
    ...
    expect(compose).toContain('PRIVATE_AGENT_CARRY_SHADOW_OBSERVER_ENABLED: "true"');
    expect(compose).toContain('PRIVATE_AGENT_CARRY_SHADOW_OBSERVER_INTERVAL_MS: "60000"');
    expect(compose).toContain('PRIVATE_AGENT_CARRY_SHADOW_OBSERVER_STALL_MS: "180000"');
    expect(compose).toContain('PRIVATE_AGENT_CARRY_SHADOW_OBSERVER_ASSETS: "BTC,ETH,SOL"');
    expect(compose).toContain('PRIVATE_AGENT_CARRY_SHADOW_QUALIFICATION_SAMPLES: "3"');
    expect(compose).toContain('PRIVATE_AGENT_CARRY_SHADOW_QUALIFICATION_MAX_AGE_MS: "600000"');
    expect(compose).toContain('PRIVATE_AGENT_CARRY_MAX_MARKET_DATA_SKEW_MS: "2000"');
    expect(compose).toContain('PRIVATE_AGENT_CARRY_MAX_INDEX_PRICE_DIVERGENCE_BPS: "25"');
    expect(compose).toContain('PRIVATE_AGENT_CARRY_MAX_MARK_PRICE_DIVERGENCE_BPS: "50"');
  ...
  it("pins an explicitly enabled capped Carry qualification runtime", () => {
    ...
    setTestEnv({
      ...
      GHOLA_CARRY_MAX_INDEX_PRICE_DIVERGENCE_BPS: "12",
      GHOLA_CARRY_MAX_MARK_PRICE_DIVERGENCE_BPS: "24",
    });
    const compose = buildPhalaWorkerCompose();
    ...
    expect(compose).toContain('PRIVATE_AGENT_CARRY_MAX_INDEX_PRICE_DIVERGENCE_BPS: "12"');
    ...
    expect(compose).toContain('PRIVATE_AGENT_CARRY_MONITOR_STALL_MS: "15000"');
    expect(compose).toContain('PRIVATE_AGENT_CARRY_EXECUTION_STALL_MS: "6000"');
    expect(compose).toContain('PRIVATE_AGENT_CARRY_SHADOW_OBSERVER_STALL_MS: "180000"');
