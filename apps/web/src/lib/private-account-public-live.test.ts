...
describe("public private-account live Phoenix access", () => {
  ...
  it("submits Coinbase public live orders to the Coinbase connector endpoint", async () => {
    const calls: Array<{ url: string; init?: RequestInit; body: Record<string, unknown> }> = [];
    const fetchImpl = async (input: URL | string, init?: RequestInit) => {
      calls.push({
        url: String(input),
      ...
      return new Response(JSON.stringify({
        status: "submitted",
        result_commitment: "coinbase_result_abc",
        provider_ref_commitment: "coinbase_provider_ref_abc",
      }), {
        status: 202,
        headers: { "content-type": "application/json" },
      });
    ...
    const result = await submitPublicLiveCoinbaseOrder({
      env: {
        GHOLA_PRIVATE_AGENT_EXECUTION_URL: "https://generic-worker.example",
        GHOLA_PRIVATE_AGENT_EXECUTION_TOKEN: "generic-token",
        GHOLA_CONNECTOR_COINBASE_STYLE_PROVIDER_URL: "https://coinbase-worker.example",
        GHOLA_CONNECTOR_COINBASE_STYLE_PROVIDER_TOKEN: "coinbase-token",
      },
      fetchImpl: fetchImpl as typeof fetch,
      allocation_commitment: "coinbase_omnibus_allocation_abc",
      omnibus_allocation: {
        venue_id: "coinbase_advanced",
        status: "allocated",
      },
      policy_commitment: "public_live_coinbase_policy_abc",
      body: {
        ack_live_order: true,
        work_order_commitment: "public_live_coinbase_work_order_abc",
        order_summary: {
          venue_id: "coinbase_advanced",
          market: "SOL-USD",
          notional_bucket: "5",
        ...
        encrypted_execution_instruction_bundle: {
          ...
          ciphertext: "ciphertext-ciphertext-ciphertext",
          recipient: "phala:cvm:test",
          aad: "ghola/private-execution-instruction-v1|work_order:public_live_coinbase_work_order_abc|venue:coinbase_advanced|recipient:phala:cvm:test",
        },
      },
    });
    ...
    expect("error" in result).toBe(false);
    expect(calls).toHaveLength(1);
    expect(calls[0].url).toBe("https://coinbase-worker.example/venues/coinbase/orders");
    expect(calls[0].body).toMatchObject({
    ...
    expect(JSON.stringify(calls[0].body)).not.toContain("private_key");
    expect(JSON.stringify(calls[0].body)).not.toContain("api_private_key");
  });
});
