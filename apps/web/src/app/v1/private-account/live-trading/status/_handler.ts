...
type LiveTradingStatusDependencies = {
...
async function handleGet(dependencies: LiveTradingStatusDependencies) {
  const [reports, capitalFreeProofs, workerReadiness] = await Promise.all([
    ...
    getPooledWorkerReadiness(process.env, dependencies.fetchImpl),
  ]);
  ...
  const byoGlobalFailures = privateAccountByoGlobalFailures(process.env);
  ...
  const byoVenues = VENUES.map((venue) => privateAccountByoVenueGate(venue.id, process.env));
...
function pooledLiveGateFailures(
  ...
  return [...new Set(privateAccountByoGlobalFailures(env).concat(workerReadiness.reason_codes))];
}
