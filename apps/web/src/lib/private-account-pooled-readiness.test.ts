...
describe("pooled worker per-venue readiness gate", () => {
  ...
  it("uses the Coinbase connector endpoint when Coinbase is the public primary venue", async () => {
    const urls: string[] = [];
    const fetchImpl = (async (url: RequestInfo | URL) => {
      ...
      return new Response(JSON.stringify({
        ...
        reason_codes: [],
        checked_at: "2026-07-13T07:06:38.812Z",
      }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    ...
    const readiness = await getPooledWorkerReadiness({
      GHOLA_PUBLIC_LIVE_PRIMARY_VENUE: "coinbase",
      ...
      GHOLA_CONNECTOR_COINBASE_STYLE_PROVIDER_TOKEN: "coinbase-worker-token",
    }, fetchImpl);

    expect(urls).toEqual(["https://coinbase-worker.example/venues/pools/readiness"]);
    expect(readiness.venues.coinbase.ready).toBe(true);
  });
});
