...
use ghola_gpu_provider::{relay_ws::associated_data, InferenceRunner, ProviderConfig};
...
async fn sealed_round_trip_mock_nitro() -> Result<()> {
    ...
    let provider_handle = tokio::spawn({
        ...
        async move {
            ...
            let _ = ghola_gpu_provider::run_with_runner(cfg, runner).await;
        }
    });
    ...
    let parsed: ghola_gpu_provider::InferenceResponseWithReceipt =
...
fn canonical_body_json(r: &ghola_gpu_provider::ReceiptV1) -> String {
    fn esc(s: &str) -> String {
