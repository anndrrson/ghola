...
`GET /health` should also include `ohttp_enabled=true`,
`ohttp_x402_gateway_enabled=true`, and
`ohttp_x402_upstream_configured=true` once the gateway can blind x402 traffic.

If the gateway will also serve Railgun x402, set the upstream cloud base URL on
`ghola-relay`:
...
Railgun x402 callers in `apps/web` use the same relay automatically when
`NEXT_PUBLIC_OHTTP_RELAY_URL` is set. Custom callers may still pass an explicit
`ohttpRelay` to `fetchWithRailgunX402`. The client uses OHTTP for both the
initial 402 challenge and the paid retry, and rejects OHTTP tunneling to any
path except `/v1/chat/completions`.
...
For x402, also test a BHTTP `POST /v1/chat/completions` capsule and confirm
the decapsulated response preserves only `content-type`, `payment-required`,
`x-payment-required`, `payment-response`, and `x-payment-response` headers.

The funded Railgun canary can exercise this path directly:
...
GHOLA_OHTTP_RELAY_URL="$NEXT_PUBLIC_OHTTP_RELAY_URL" \
GHOLA_RELAY_BASE_URL=https://ghola-relay.onrender.com \
  scripts/canary/railgun-x402-funded-canary.sh --replay-check
```

In OHTTP mode the canary verifies both legs: the unpaid 402 challenge returns a
Railgun `request_hash`, then the funded proof is submitted through the same
relay path.
