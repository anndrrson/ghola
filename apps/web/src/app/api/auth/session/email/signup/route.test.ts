...
function jsonRequest(body: unknown, origin = "https://ghola.test") {
  return new NextRequest("https://ghola.test/api/auth/session/email/signup", {
    method: "POST",
...
describe("email session sign-up proxy", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("rejects cross-site sign-up attempts before contacting auth upstream", async () => {
