...
const WORKER_KEYS = [
  ...
  "PRIVATE_AGENT_CONSUMER_DATABASE_URL",
  "PRIVATE_AGENT_TRADING_CONTROL_TOKEN",
  "PRIVATE_AGENT_RECONCILIATION_INGEST_TOKEN",
  "PRIVATE_AGENT_CONSUMER_WITHDRAWAL_DISPATCH_URL",
  "PRIVATE_AGENT_CONSUMER_WITHDRAWAL_DISPATCH_TOKEN",
  "PRIVATE_AGENT_CONSUMER_SOLANA_RPC_URL",
  "PRIVATE_AGENT_CONSUMER_SOLANA_USDC_TREASURY_RECIPIENT",
  "PRIVATE_AGENT_CONSUMER_SOLANA_USDC_MINT",
  "PRIVATE_AGENT_VERCEL_SPEND_WEBHOOK_SECRET",
...
const ALIASES = {
  ...
  PRIVATE_AGENT_CONSUMER_DATABASE_URL: ["GHOLA_CONSUMER_DATABASE_URL", "GHOLA_PRIVATE_ACCOUNT_DATABASE_URL", "DATABASE_URL"],
  PRIVATE_AGENT_TRADING_CONTROL_TOKEN: ["GHOLA_TRADING_CONTROL_TOKEN"],
  PRIVATE_AGENT_RECONCILIATION_INGEST_TOKEN: ["GHOLA_RECONCILIATION_INGEST_TOKEN"],
  PRIVATE_AGENT_CONSUMER_WITHDRAWAL_DISPATCH_URL: ["GHOLA_CONSUMER_WITHDRAWAL_DISPATCH_URL"],
  PRIVATE_AGENT_CONSUMER_WITHDRAWAL_DISPATCH_TOKEN: ["GHOLA_CONSUMER_WITHDRAWAL_DISPATCH_TOKEN"],
  PRIVATE_AGENT_CONSUMER_SOLANA_RPC_URL: ["GHOLA_CONSUMER_SOLANA_RPC_URL", "GHOLA_SOLANA_RPC_URL"],
  PRIVATE_AGENT_CONSUMER_SOLANA_USDC_TREASURY_RECIPIENT: ["GHOLA_CONSUMER_SOLANA_USDC_TREASURY_RECIPIENT"],
  PRIVATE_AGENT_CONSUMER_SOLANA_USDC_MINT: ["GHOLA_CONSUMER_SOLANA_USDC_MINT"],
  PRIVATE_AGENT_VERCEL_SPEND_WEBHOOK_SECRET: ["GHOLA_VERCEL_SPEND_WEBHOOK_SECRET"],
...
if (String(merged.GHOLA_HYPERLIQUID_LIVE_MODE || "").trim()) {
  fail("GHOLA_HYPERLIQUID_LIVE_MODE is deprecated; set PRIVATE_AGENT_HYPERLIQUID_LIVE_MODE explicitly.");
}
...
const mainnetLive = out.PRIVATE_AGENT_HYPERLIQUID_ALLOW_MAINNET === "true" ||
  out.PRIVATE_AGENT_HYPERLIQUID_LIVE_MODE === "full_ticket";
if (mainnetLive) {
  const exactLive = [
  ...
  try {
    const privateKey = createPrivateKey({
      key: Buffer.from(out.PRIVATE_AGENT_FUNDING_SIGNING_KEY || "", "base64"),
      format: "der",
      type: "pkcs8",
    });
    derivedSigner = createPublicKey(privateKey).export({ format: "der", type: "spki" }).toString("base64");
  } catch {
    fail("PRIVATE_AGENT_FUNDING_SIGNING_KEY must be valid base64 PKCS8 Ed25519 material.");
  }
  if (!signerPins.includes(derivedSigner)) {
    fail("GHOLA_FUNDING_WORKER_SIGNER_KEYS_B64 must pin the configured funding signer.");
  }
}
if (missing.length) {
