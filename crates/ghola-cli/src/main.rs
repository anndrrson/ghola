...
enum Commands {
    ...
    Relay {
        /// Bind address (overrides GHOLA_RELAY_BIND env var)
        #[arg(short, long)]
        bind: Option<String>,
...
async fn cmd_relay(bind: Option<String>, dev: bool) {
    ...
    if let Some(addr) = bind {
        std::env::set_var("GHOLA_RELAY_BIND", &addr);
    }
    if dev {
        std::env::set_var("GHOLA_DEV_MODE", "true");
    }

    init_tracing();

    if let Err(e) = ghola_relay::run_relay().await {
        eprintln!("Error: {}", e);
        std::process::exit(1);
    }
}
...
fn cmd_config(
    ...
    if changed {
        ...
        if config_path.exists() {
            ...
            println!("  GHOLA_RELAY_URL, GHOLA_MCP_PUBKEY, GHOLA_DEVICE_PUBKEY");
        }
    }
}
...
async fn cmd_gpu_serve(
    ...
    // 3. Build model info
    let model_infos: Vec<ghola_assistant_types::ProviderModelInfo> = discovered_models
        .iter()
        .map(|model_id| ghola_assistant_types::ProviderModelInfo {
    ...
    loop {
        ...
        let auth_msg = ghola_assistant_types::AuthMessage {
            pubkey: pubkey_b58.clone(),
            timestamp,
            nonce,
            role: ghola_assistant_types::ConnectionRole::GpuProvider,
        };
        ...
        let auth_payload = ghola_assistant_types::AuthPayload {
            message: auth_msg,
            signature: sig_b64,
        };
        ...
        let advertise = ghola_assistant_types::Envelope::new(ghola_assistant_types::MessageType::ProviderAdvertise(
            ghola_assistant_types::ProviderAdvertisePayload {
                name: name.clone(),
        ...
        match ws_read.next().await {
            Some(Ok(msg)) => {
                let text = msg.to_text().unwrap_or("");
                if let Ok(env) = serde_json::from_str::<ghola_assistant_types::Envelope>(text) {
                    match &env.message {
                        ghola_assistant_types::MessageType::ProviderAdvertiseAck(ack) => {
                            if ack.accepted {
                                println!(
                                    "Provider registered. {}",
        ...
        let heartbeat_handle = tokio::spawn(async move {
            ...
            loop {
                interval.tick().await;
                let hb = ghola_assistant_types::Envelope::new(
                    ghola_assistant_types::MessageType::ProviderHeartbeat(
                        ghola_assistant_types::ProviderHeartbeatPayload {
                ...
                msg = ws_read.next() => {
                    match msg {
                        Some(Ok(ws_msg)) => {
                            ...
                            if let Ok(envelope) = serde_json::from_str::<ghola_assistant_types::Envelope>(text) {
                                match &envelope.message {
                                    ghola_assistant_types::MessageType::InferenceRequest(payload) => {
                                    ...
                                    ghola_assistant_types::MessageType::Ping => {
                                        let pong = envelope.response(ghola_assistant_types::MessageType::Pong);
...
async fn handle_inference_request(
    inference_url: &str,
    envelope: &ghola_assistant_types::Envelope,
    payload: &ghola_assistant_types::InferenceRequestPayload,
        ...
        Ok(response) if response.status().is_success() => {
            if payload.stream {
                                    ...
                                    if let Ok(json) = serde_json::from_str::<serde_json::Value>(data) {
                                            ...
                                            .and_then(|c| c.as_str())
                                        {
                                            ...
                                            let chunk_env = envelope.response(
                                                ghola_assistant_types::MessageType::InferenceStreamChunk(
                                                    ghola_assistant_types::InferenceStreamChunk {
                ...
                let end_env = envelope.response(
                    ghola_assistant_types::MessageType::InferenceStreamEnd(
                        ghola_assistant_types::InferenceStreamEnd {
                            job_id: payload.job_id.clone(),
                            input_tokens,
                            output_tokens,
                            latency_ms,
                ...
                match response.json::<serde_json::Value>().await {
                    Ok(json) => {
                        ...
                        let resp_env = envelope.response(
                            ghola_assistant_types::MessageType::InferenceResponse(
                                ghola_assistant_types::InferenceResponsePayload {
                                    job_id: payload.job_id.clone(),
                                    text,
                                    input_tokens,
                                    output_tokens,
...
fn send_error(
    ...
    let err_env = envelope.response(ghola_assistant_types::MessageType::Error(
        ghola_assistant_types::ErrorPayload {
