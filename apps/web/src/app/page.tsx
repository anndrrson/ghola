...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#05070b] pt-16 text-[#eef1f8]">
      <AuthModal
        mode={authMode}
        open={authOpen}
      ...
      <section className="relative overflow-hidden">
        ...
        />
        <div className="relative mx-auto flex min-h-[calc(100svh-4rem)] max-w-5xl flex-col items-center justify-center gap-7 px-4 py-20 text-center sm:px-6">
          <div className="inline-flex items-center gap-2 rounded-full border border-[#263752] bg-[#09101a]/80 px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.18em] text-[#8fb9e8] shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] backdrop-blur">
            Private execution for live markets
          </div>
          <h1 className="max-w-4xl font-display text-5xl font-semibold leading-[0.98] tracking-[-0.045em] text-[#f6f8ff] [text-shadow:0_0_80px_rgba(90,167,255,0.25)] sm:text-7xl lg:text-[5.5rem]">
            Trade like a ghost.
          </h1>
          <p className="max-w-xl text-base leading-7 text-[#9aa8bf] sm:text-lg">
            Give an agent a hard spending limit. Ghola handles execution without exposing your main wallet or withdrawal access.
          </p>
      ...
      <section className="border-t border-[#141d2e]">
        <div className="mx-auto grid max-w-6xl gap-4 px-4 py-14 sm:px-6 md:grid-cols-3">
          <PillarCard
            icon={LockKeyhole}
            title="Scoped access"
            body="Trading authority only. No seed phrase and no withdrawal permission."
          />
          <PillarCard
            icon={Gauge}
            title="Bounded by design"
            body="Size, slippage, and loss limits are checked before an order can leave."
          />
          <PillarCard
            icon={ReceiptText}
            title="Receipts, not promises"
            body="Every plan is committed before execution and leaves an auditable trail."
          />
        </div>
      </section>
