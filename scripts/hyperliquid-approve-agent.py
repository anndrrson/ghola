...
signing the ApproveAgent action programmatically (in an ephemeral shell)
is the intended flow — no Chrome extension or wallet bridge required.
...
master key, and prints the managed-accounts JSON entry for the sealed
install. Nothing is written to disk; the master key should go to cold
storage immediately afterwards.
"""
...
def main() -> int:
    ...
    base_url = (
        constants.TESTNET_API_URL if args.network == "testnet" else constants.MAINNET_API_URL
    )
    ...
    result = exchange.approve_agent_key(agent.address, args.agent_name) if hasattr(
        exchange, "approve_agent_key"
    ...
    print("managed-accounts entry (stdout below) — pipe straight into the sealed install:", file=sys.stderr)
    print(json.dumps({
        "accounts": [{
