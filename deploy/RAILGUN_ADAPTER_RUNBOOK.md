...
This runbook wires the `railgun_evm_shielded` adapter into `ghola-cloud`
without changing adapter or cloud code. The adapter is fail-closed:
`ghola-cloud` only advertises the rail when the adapter URL, auth token,
...
```bash
RAILGUN_ADAPTER_AUTH_TOKEN=<shared-secret-with-ghola-cloud>
...
RAILGUN_EVM_RECIPIENT=<railgun-0zk-recipient> \
RAILGUN_EVM_ADAPTER_AUTH_TOKEN=<shared-secret-with-ghola-cloud> \
...
needed by `ghola-cloud`. It redacts the auth token by default. Use
...
Apply the emitted values to the ghola-cloud secret store, then redeploy
ghola-cloud.
...
After ghola-cloud redeploys, run the end-to-end health canary:

```bash
RAILGUN_EVM_ADAPTER_URL=https://<adapter-host> \
RAILGUN_EVM_RECIPIENT=<railgun-0zk-recipient> \
GHOLA_BASE_URL=https://<thumper-cloud-host> \
...
- `ghola-cloud /health/payments` reports
...
`ghola-cloud` stores the Railgun replay key in `x402_payments`.
...
Unset only the cloud-side rail enablement values and redeploy ghola-cloud:
