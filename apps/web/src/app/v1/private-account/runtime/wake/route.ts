...
const MAX_WAKE_LEASE_MS = 5 * 60_000;
...
export async function POST(req: Request) {
  ...
  const leaseMs = Math.min(
    MAX_WAKE_LEASE_MS,
    Math.max(1, billing.required_seconds) * 1_000,
  );
  ...
  if (!coolingDown) {
    ...
    provisioning = await wakePhalaPrivateAgentForUse({
      reason: `android_qualified_trade_intent:${productId}`,
      waitForReadyMs: 0,
      leaseMs,
    });
  }
  ...
  const response = await runtimeStatus({
    ...
    retryAfterMs: coolingDown
      ? Math.max(1_000, WAKE_COOLDOWN_MS - (now - previous))
      : 1_000,
    leaseMs: coolingDown ? 0 : leaseMs,
  });
  return response;
}
