...
describe("private account trading UI derivation", () => {
  ...
  it("keeps focused onboarding to sign-in plus two visible Hyperliquid steps", () => {
    ...
    expect(carryAccountSetupSource).toContain("Use Ghola Touch ID only if you already added it here.");
    expect(carryAccountSetupSource).toContain("Optional — use Touch ID for faster sign-in on this device.");
    ...
    expect(carryAccountSetupSource).toContain("Explore routes");
    ...
    expect(carryAccountSetupSource).toContain("No deposit required");
