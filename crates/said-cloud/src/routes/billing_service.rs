...
pub async fn usage_summary(
    ...
    let query = format!(
        r#"SELECT
            ...
            COALESCE(SUM(request_count), 0)::BIGINT as total_api_calls,
            COALESCE(SUM(amount_micro_usdc), 0)::BIGINT as total_amount,
    ...
    let unsettled: i64 = sqlx::query_scalar(
        "SELECT COALESCE(SUM(amount_micro_usdc), 0)::BIGINT FROM metered_usage WHERE service_id = $1 AND settled = false",
    )
    .bind(service_id)
    .fetch_one(&state.db)
    .await?;
...
async fn process_settlements(
    ...
    let services: Vec<(Uuid, i32, Option<String>)> = sqlx::query_as(
        r#"SELECT mu.service_id,
                  COALESCE(sl.platform_fee_bps, 300)::INT,
                  sl.owner_did
    ...
    for (service_id, fee_bps, owner_did) in services {
        ...
        let (total_requests, total_amount): (i64, i64) = sqlx::query_as(
            r#"SELECT COALESCE(SUM(request_count), 0)::BIGINT, COALESCE(SUM(amount_micro_usdc), 0)::BIGINT
            FROM metered_usage
