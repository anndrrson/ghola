...
pub async fn run_cloud() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let state = AppState::new(config, pool, vault);
...
pub fn build_router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            "/api/compute/providers/me/estimate",
            post(routes::compute::estimate_earnings),
        )
        .route(
            "/api/compute/providers/me/receipts",
            get(routes::compute::get_usage_receipts),
        )
        .route(
            "/api/compute/providers/me/receipts/verify",
            get(routes::compute::verify_usage_receipts),
        )
        // Agent Rental
        .route(
            "/api/agents",
