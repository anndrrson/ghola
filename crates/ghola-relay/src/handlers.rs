...
fn relay_health_body(state: &AppState, ready_status: bool) -> serde_json::Value {
    ...
    let x402_upstream_configured = !state.config().ghola_cloud_base_url.trim().is_empty();
...
// honour `GHOLA_ALLOW_UNATTESTED=1` so local dev + the test suite
// keep working.
//
...
fn allow_unattested() -> bool {
    ...
    ghola_assistant_types::env_compat("GHOLA_ALLOW_UNATTESTED", "THUMPER_ALLOW_UNATTESTED").as_deref() == Ok("1")
}
...
pub(crate) fn handle_provider_attest(
    ...
    // Dev/staging mock path: GHOLA_ALLOW_UNATTESTED=1 + TeeKind::None.
    ...
    ProviderAttestAckPayload {
        ...
        reason: Some("accepted via GHOLA_ALLOW_UNATTESTED dev path".into()),
    }
}
...
async fn ohttp_x402_chat_completions(
    ...
    let base = state.config().ghola_cloud_base_url.trim_end_matches('/');
