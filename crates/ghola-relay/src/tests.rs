...
async fn ohttp_x402_gateway_proxies_only_payment_allowlisted_headers() {
    ...
    cfg.ohttp_key_secret_hex =
        Some("00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff".to_string());
    cfg.ghola_cloud_base_url = upstream_base;
...
fn h100_dispatch_env_setup() -> (
    ...
    let env = EnvGuard::set(&[
        ("GHOLA_ALLOW_UNATTESTED", None),
        ("GHOLA_NVIDIA_NRAS_ROOT_PEM", Some(&nras_pub_hex)),
...
fn tdx_dispatch_env_setup() -> (
    ...
    let env = EnvGuard::set(&[
        ("GHOLA_ALLOW_UNATTESTED", None),
        ("GHOLA_INTEL_TDX_ROOT_PEM", Some(&tdx_pub_hex)),
        ("GHOLA_ATTEST_SIGNING_PUB", Some(&allow_pub_hex)),
    ]);
    (tdx_sk, allow_sk, env)
}
...
async fn health_includes_private_readiness_fields() {
    ...
    let body = axum::body::to_bytes(response.into_body(), 64 * 1024)
        ...
        .expect("read body");
    let json: serde_json::Value = serde_json::from_slice(&body).expect("json body");
    assert_eq!(json["service"], "ghola-relay");
...
#[test]
fn startup_guard_blocks_dev_mode() {
    // dev_mode disables signature verification — fatal in production.
    let issues = crate::startup_issues(true, &[], false);
    assert_eq!(
        issues,
        vec![
            "GHOLA_DEV_MODE disables signature verification; must be off in production"
                .to_string()
        ]
    );
}
...
#[test]
fn startup_guard_blocks_preflight_failures() {
    let preflight = vec!["ohttp_key_missing".to_string(), "did_set_url_missing".to_string()];
    let issues = crate::startup_issues(false, &preflight, false);
    assert_eq!(issues, preflight);
}

#[test]
fn startup_guard_reports_all_issues_together() {
    let preflight = vec!["ohttp_key_missing".to_string()];
    let issues = crate::startup_issues(true, &preflight, false);
    assert_eq!(issues.len(), 2);
    assert!(issues.iter().any(|i| i.contains("GHOLA_DEV_MODE")));
    assert!(issues.iter().any(|i| i == "ohttp_key_missing"));
}

#[test]
fn startup_guard_override_bypasses_all() {
    let preflight = vec!["ohttp_key_missing".to_string()];
    let issues = crate::startup_issues(true, &preflight, true);
    assert!(issues.is_empty());
}

#[test]
fn startup_guard_clean_config_has_no_issues() {
    let issues = crate::startup_issues(false, &[], false);
    assert!(issues.is_empty());
}
