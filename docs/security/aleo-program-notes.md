...
`ghola-cloud` does not talk to the Aleo network directly. It talks
to the off-chain shielded adapter, which talks to Aleo. The mapping:

| env var (ghola-cloud) | Aleo concept | Used by |
|---|---|---|
| `SHIELDED_STABLECOIN_PROVIDER=aleo` | Discriminant for which rail to validate. | ghola-cloud verifier dispatch. |
| `SHIELDED_STABLECOIN_NETWORK=aleo:mainnet` | Target Aleo network (`aleo:mainnet` or `aleo:testnet3`). Must match `program.json` deploy target. | ghola-cloud + adapter; the adapter rejects transitions submitted against a different network. |
...
| `SHIELDED_STABLECOIN_ADAPTER_URL` | Off-chain verifier endpoint. Not an Aleo concept; the adapter is the thing that knows how to talk to an Aleo node. | ghola-cloud. |
| `SHIELDED_STABLECOIN_ADAPTER_PUBKEY` | Ed25519 pubkey signing adapter receipts. Decoupled from any Aleo key. | ghola-cloud (signature verification). |
| `SHIELDED_STABLECOIN_VERIFIER_READY` | Gate flag: `true` only when a real on-chain verifier (this program live + adapter wired to it) exists. Until then ghola-cloud reports `configured=false`. | ghola-cloud health endpoint. |
...
- The on-chain program proves: "someone who owned a USDC.a record
  ...
  notion of x402.
- The adapter proves to `ghola-cloud`: "I fetched transition `t`,
...
- `ghola-cloud` does **not** verify the Aleo proof itself. It
  ...
  + a trusted attestor", not "natively verified in ghola-cloud."
...
gated by env-var flips on the `ghola-cloud` Render service plus
...
- ghola-cloud `/health/payments` reports `aleo_usdcx_shielded.configured=false`.
...
ghola-cloud-native Rust check using `snarkVM` directly, or (b) lift
verification onto a Solana-native zk-verifier program that
ghola-cloud can call as part of its existing settlement path, the
...
2. Run adapter + native verifier in parallel for ≥2 weeks. Cross-check
   ...
   alert on divergence.
3. Flip ghola-cloud to native-only.
