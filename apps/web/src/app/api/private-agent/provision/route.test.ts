...
const STRONG_TOKEN = "operator-provision-token-00000000000000000001";
...
describe("private-agent provision route", () => {
  it("blocks localhost provisioning before any external request", async () => {
    ...
    process.env.GHOLA_PRIVATE_AGENT_PROVISION_TOKEN = STRONG_TOKEN;
  ...
  it("requires the explicit release-mutation flag after operator authentication", async () => {
    ...
    const response = await POST(new Request("https://ghola.test/api/private-agent/provision", {
      method: "POST",
      headers: { authorization: `Bearer ${STRONG_TOKEN}` },
    }) as never);
  ...
  it("is the explicit release-mutation entrypoint", () => {
    expect(POST.toString()).toContain("allowReleaseMutation: true");
  });

  it("rejects a weak configured bearer before checking mutation state", async () => {
