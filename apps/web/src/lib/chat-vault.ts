...
export interface ChatVault {
  ...
  /**
   * Sign arbitrary bytes with the user's identity Ed25519 key. Other
   * vault-aware helpers (e.g. chat-history-store) need this to seal
   * envelopes whose `signBody` callback isn't already wired up. Always
   * goes through the same signer the vault was constructed with.
   */
...
export function createChatVault(opts: ChatVaultOptions): ChatVault {
  ...
  return {
    ...
    async sealUserMessage(sessionId, plaintext) {
      ...
      // only the user's own derived X25519 secret can. AD = (sessionId,
      // role) so re-shuffling envelopes between sessions on the server
      // fails AEAD verification. The recipient X25519 keypair is
      // derived deterministically from a Turnkey-signed challenge so
      // the user can recover it on any device by re-signing.
      const recipient = await deriveVaultX25519Keypair(opts.signBytes);
