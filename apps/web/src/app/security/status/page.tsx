...
const INITIAL: ChecksState = [
  ...
  { label: "Bootstrap CSP enforcement", state: "pending", detail: "probing / response headers…" },
...
function cspAllowsNextBootstrap(csp: string): boolean {
...
export default function SecurityStatusPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (missing.length === 0) {
          next[5] = {
            ...
            state: "ok",
            detail: "all 4 required headers present (CSP: enforcing)",
            evidence: `XFO=${xfo}; XCTO=${xcto}; HSTS=${hsts?.slice(0, 30)}…; CSP=enforce`,
          };
        } else {
          next[5] = {
            label: "Security response headers",
      ...
      // 9. Bootstrap CSP enforcement — CSP is generated by the proxy. Next's
      //    statically prerendered app pages need an allowed inline bootstrap
      //    path; otherwise production can return 200 while rendering only a
      //    blank shell in browsers.
      try {
        ...
        if (!res.ok) {
          next[8] = {
            label: "Bootstrap CSP enforcement",
            state: "warn",
            detail: `homepage probe returned HTTP ${res.status}`,
          };
        } else if (!csp) {
          next[8] = {
            label: "Bootstrap CSP enforcement",
        ...
        } else if (!cspAllowsNextBootstrap(csp)) {
          next[8] = {
            label: "Bootstrap CSP enforcement",
            state: "fail",
            detail: "script-src does not allow Next bootstrap scripts",
            evidence: scriptSrc || "script-src missing",
          };
        } else {
          next[8] = {
            label: "Bootstrap CSP enforcement",
            state: "ok",
            detail: `bootstrap-compatible CSP active (${csp.length} bytes)`,
            evidence: scriptSrc,
          };
        }
      } catch (err) {
