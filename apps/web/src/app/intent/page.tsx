...
export default function IntentPage() {
  ...
  const [goal, setGoal] = useState(
    "Find a verified option and prepare a private checkout for a useful AI service."
  );
  ...
  return (
    <main className="min-h-screen bg-[#08090d] pt-16 text-[#eef1f8]">
      <section className="border-b border-[#151b26] px-5 py-10 sm:px-8 lg:px-10">
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[1fr_22rem] lg:items-end">
          <div>
            ...
            <h1 className="mt-5 max-w-3xl text-4xl font-medium tracking-tight text-[#f6f8ff] sm:text-5xl">
              Let Ghola find, approve, and pay.
            </h1>
            <p className="mt-4 max-w-2xl text-base leading-7 text-[#aab5c8]">
              A private checkout flow for agentic commerce: options first,
              clear approval before anything leaves Ghola, then a redacted receipt.
            </p>
          </div>
          <div className="rounded-md border border-[#1e2a3a] bg-[#0c0f15] p-4">
            <p className="text-xs uppercase tracking-[0.16em] text-[#6f7d9a]">
              Private rail
            </p>
            <p className="mt-2 text-sm font-medium text-[#eef1f8]">
              {privateSummary.privateSpendReady
                ? "No-funds canary passed"
                : "Private rail setup required"}
            </p>
            <p className="mt-2 text-xs leading-5 text-[#8b95a8]">
              {privateSummary.privateSpendReady
                ? "Aleo USDCx is configured and fail-closed. Funded proof remains an explicit release gate."
                : privateSummary.detail}
            </p>
            <p className="mt-3 text-xs text-[#7f8ca3]">
              Funded proof:{" "}
              <span className="text-[#d6deec]">
        ...
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
          <div className="space-y-5">
            {!thumperAuth.loading && !thumperAuth.authenticated && (
              <div className="rounded-md border border-[#4d2e22] bg-[#1a100d] px-4 py-3 text-sm text-[#ffc7aa]">
                Sign in to create quotes and receipts.
              </div>
            )}
            ...
            <section className="rounded-md border border-[#1e2a3a] bg-[#0c0f15]">
              <div className="border-b border-[#1e2a3a] px-5 py-4">
                <h2 className="text-sm font-medium text-[#f6f8ff]">What should Ghola do?</h2>
              </div>
              <div className="grid gap-5 p-5">
                <textarea
                  value={goal}
                  onChange={(event) => setGoal(event.target.value)}
                  className="min-h-28 resize-y rounded-md border border-[#263449] bg-[#08090d] px-3 py-3 text-sm leading-6 text-[#eef1f8] outline-none transition focus:border-[#3da8ff]"
                  maxLength={1200}
                />
                <div className="grid gap-4 md:grid-cols-[12rem_1fr]">
                  <label className="grid gap-2">
                    <span className="text-xs font-medium uppercase tracking-[0.16em] text-[#6f7d9a]">
                      Budget
                    </span>
                    <input
                      value={budgetUsd}
                      ...
                      inputMode="decimal"
                      className="h-11 rounded-md border border-[#263449] bg-[#08090d] px-3 text-sm text-[#eef1f8] outline-none transition focus:border-[#3da8ff]"
                    />
                  </label>
                  <div className="grid gap-2">
                    ...
                    <div className="grid h-11 grid-cols-2 overflow-hidden rounded-md border border-[#263449] bg-[#08090d]">
                ...
                <button
                  ...
                  className="inline-flex h-11 items-center justify-center gap-2 rounded-md bg-[#3da8ff] px-4 text-sm font-medium text-[#07111d] transition hover:bg-[#67bbff] disabled:cursor-not-allowed disabled:opacity-50 sm:w-fit"
                >
            ...
            <section className="rounded-md border border-[#1e2a3a] bg-[#0c0f15]">
              ...
              <div className="divide-y divide-[#151b26]">
                {offers.length === 0 ? (
                  <p className="px-5 py-8 text-sm text-[#7f8ca3]">
                    Ask Ghola to discover curated demo offers, x402 services, and future merchant checkouts.
                  </p>
                ) : (
                  offers.map((offer) => (
                    <button
                      ...
                      className={`grid w-full gap-3 px-5 py-4 text-left transition md:grid-cols-[1fr_9rem_8rem] md:items-center ${
          ...
          <aside className="h-fit rounded-md border border-[#1e2a3a] bg-[#0c0f15] p-5">
            ...
            <div className="mt-6 border-t border-[#1e2a3a] pt-5 text-xs leading-5 text-[#7f8ca3]">
              <p>
                Private mode uses USDCx when ready and refuses public fallback.
                Public USDC remains available only when the user selects Open.
              </p>
              <Link
                href="/private-balance"
                className="mt-4 inline-flex items-center gap-2 text-[#3da8ff] hover:text-[#67bbff]"
              >
