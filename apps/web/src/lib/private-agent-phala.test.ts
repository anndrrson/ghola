...
const TEST_ENV_KEYS = [
  ...
  "GHOLA_CONSUMER_WITHDRAWAL_DISPATCH_URL",
  "GHOLA_CONSUMER_WITHDRAWAL_DISPATCH_TOKEN",
...
describe("private-agent Phala provisioning", () => {
  it("builds a no-plaintext worker compose with dstack quote binding", () => {
    ...
    expect(compose).toContain(
      'PRIVATE_AGENT_CONSUMER_DATABASE_URL: "${PRIVATE_AGENT_CONSUMER_DATABASE_URL:-}"',
    );
    expect(compose).toContain(
      'PRIVATE_AGENT_TRADING_CONTROL_TOKEN: "${PRIVATE_AGENT_TRADING_CONTROL_TOKEN:-}"',
    );
    expect(compose).toContain(
      'PRIVATE_AGENT_RECONCILIATION_INGEST_TOKEN: "${PRIVATE_AGENT_RECONCILIATION_INGEST_TOKEN:-}"',
    );
    expect(compose).toContain(
      'PRIVATE_AGENT_CONSUMER_WITHDRAWAL_DISPATCH_TOKEN: "${PRIVATE_AGENT_CONSUMER_WITHDRAWAL_DISPATCH_TOKEN:-}"',
    );
    expect(compose).toContain(
      'PRIVATE_AGENT_CONSUMER_SOLANA_RPC_URL: "${PRIVATE_AGENT_CONSUMER_SOLANA_RPC_URL:-}"',
    );
    expect(compose).toContain(
      'PRIVATE_AGENT_CONSUMER_SOLANA_USDC_TREASURY_RECIPIENT: "${PRIVATE_AGENT_CONSUMER_SOLANA_USDC_TREASURY_RECIPIENT:-}"',
    );
