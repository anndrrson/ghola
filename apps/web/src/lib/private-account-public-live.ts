...
const PUBLIC_LIVE_COINBASE_NOTIONAL_BUCKETS = new Set(["5", "10", "25", "50", "100", "250"]);
...
export interface PublicLivePhoenixOrderSummary {
  venue_id: "phoenix";
  market: "SOL-PERP";
  notional_bucket: "5";
  side: "buy" | "sell";
...
export interface PublicLiveCoinbaseSubmitInput {
  work_order_commitment?: unknown;
  encrypted_execution_instruction_bundle?: unknown;
  omnibus_allocation?: unknown;
  allocation_commitment?: unknown;
  policy_commitment?: unknown;
  ack_live_order?: unknown;
  order_summary?: unknown;
...
export interface PublicLiveCoinbaseOrderSummary {
...
export function publicLiveCoinbaseSessionPolicy(input: {
  policy_commitment?: string | null;
  market_allowlist?: string[];
  max_notional_bucket?: string;
  max_order_count?: number;
  ...
  const policy = {
    market_allowlist: input.market_allowlist?.length ? input.market_allowlist : ["SOL-USD", "BTC-USD", "ETH-USD"],
    max_notional_bucket: input.max_notional_bucket || PUBLIC_LIVE_MAX_NOTIONAL_BUCKET,
    max_order_count: input.max_order_count ?? PUBLIC_LIVE_MAX_ORDER_COUNT,
    kill_switch: false,
  };
  return {
    ...policy,
    policy_commitment: input.policy_commitment || gholaCommitment("public_live_coinbase_session_policy", policy),
  };
}
...
export async function submitPublicLivePhoenixOrder(input: {
  ...
  const orderSummary = validatePublicLivePhoenixOrderSummary(value.order_summary, workOrderCommitment);
  if ("error" in orderSummary) return { error: orderSummary.error, status: 400 };
  const encryptedInstruction = value.encrypted_execution_instruction_bundle;
  if (!isSealedExecutionInstructionBundle(encryptedInstruction, workOrderCommitment, "phoenix")) {
  ...
  const payload = {
    ...
    order_summary: orderSummary.summary,
...
export function validatePublicLivePhoenixOrderSummary(
  value: unknown,
  expectedWorkOrderCommitment: string,
):
  | { ok: true; summary: PublicLivePhoenixOrderSummary }
  | {
      ok: false;
      error:
        | "order_summary_required"
        | "order_summary_work_order_mismatch"
        | "order_summary_venue_invalid"
        | "order_summary_market_invalid"
        | "order_summary_notional_invalid"
        | "order_summary_side_invalid";
    } {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return { ok: false, error: "order_summary_required" };
  }
  const record = value as Record<string, unknown>;
  if (stringValue(record.work_order_commitment) !== expectedWorkOrderCommitment) {
    return { ok: false, error: "order_summary_work_order_mismatch" };
  }
  if (stringValue(record.venue_id) !== "phoenix") {
    return { ok: false, error: "order_summary_venue_invalid" };
  }
  if (stringValue(record.market).toUpperCase() !== "SOL-PERP") {
    return { ok: false, error: "order_summary_market_invalid" };
  }
  if (stringValue(record.notional_bucket) !== PUBLIC_LIVE_MAX_NOTIONAL_BUCKET) {
    return { ok: false, error: "order_summary_notional_invalid" };
  }
  const side = stringValue(record.side).toLowerCase();
  if (side !== "buy" && side !== "sell") {
    return { ok: false, error: "order_summary_side_invalid" };
  }
  return {
    ok: true,
    summary: {
      venue_id: "phoenix",
      market: "SOL-PERP",
      notional_bucket: PUBLIC_LIVE_MAX_NOTIONAL_BUCKET,
      side,
      work_order_commitment: expectedWorkOrderCommitment,
    },
  };
}

export async function submitPublicLiveCoinbaseOrder(input: {
  body: PublicLiveCoinbaseSubmitInput;
  omnibus_allocation: unknown;
  allocation_commitment: string;
  policy_commitment?: string | null;
  env?: Record<string, string | undefined>;
  ...
  const orderSummary = validatePublicLiveCoinbaseOrderSummary(value.order_summary, workOrderCommitment);
  if ("error" in orderSummary) return { error: orderSummary.error, status: 400 };
  const encryptedInstruction = value.encrypted_execution_instruction_bundle;
  if (!isSealedExecutionInstructionBundle(encryptedInstruction, workOrderCommitment, "coinbase_advanced")) {
    return { error: "encrypted_execution_instruction_required" as const, status: 400 };
  }
  const cfg = publicLiveWorkerConfig(input.env ?? process.env, "coinbase");
  if (!cfg.url) return { error: "private_agent_worker_endpoint_missing" as const, status: 503 };

  const sessionPolicy = publicLiveCoinbaseSessionPolicy({
    policy_commitment: input.policy_commitment ?? null,
    max_notional_bucket: orderSummary.summary.notional_bucket,
  });
  const workerPath = "/venues/coinbase/orders";
  const payload = {
    version: 1,
...
export function validatePublicLiveCoinbaseOrderSummary(
  value: unknown,
  expectedWorkOrderCommitment: string,
):
  | { ok: true; summary: PublicLiveCoinbaseOrderSummary }
  | {
      ok: false;
      error:
        | "order_summary_required"
  ...
  if (stringValue(record.venue_id) !== "coinbase_advanced") {
    return { ok: false, error: "order_summary_venue_invalid" };
  }
  const market = stringValue(record.market).toUpperCase();
  if (market !== "SOL-USD" && market !== "BTC-USD" && market !== "ETH-USD") {
    return { ok: false, error: "order_summary_market_invalid" };
  }
  const notionalBucket = stringValue(record.notional_bucket);
  if (!PUBLIC_LIVE_COINBASE_NOTIONAL_BUCKETS.has(notionalBucket)) {
    return { ok: false, error: "order_summary_notional_invalid" };
  }
  const side = stringValue(record.side).toLowerCase();
  if (side !== "buy" && side !== "sell") {
...
function publicLiveWorkerConfig(env: Record<string, string | undefined>, venue: "phoenix" | "coinbase" = "phoenix") {
  const venueUrls = venue === "coinbase"
    ? ["GHOLA_CONNECTOR_COINBASE_STYLE_PROVIDER_URL"]
    : ["GHOLA_CONNECTOR_SOLANA_PERPS_MARKET_URL"];
  const venueTokens = venue === "coinbase"
...
function isSealedExecutionInstructionBundle(
  value: unknown,
  workOrderCommitment: string,
  venueId: "phoenix" | "coinbase_advanced",
  ...
  return (
    ...
    record.aad.includes(`venue:${venueId}`) &&
    record.aad.includes(`work_order:${workOrderCommitment}`)
  );
}
