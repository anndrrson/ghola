...
import { terminalLiveExecutionSubjectScope } from "@/lib/terminal-live-execution-journal";
...
export type TradingSessionDependencies = {
  fetchImpl: typeof fetch;
  fetchSessionUserImpl: typeof fetchSessionUser;
};

/** Explicit dependency seam for unit tests; transport policy still requires the test-only brand. */
export function createTradingSessionPost(dependencies: TradingSessionDependencies) {
  return (req: NextRequest) => handlePost(req, dependencies);
}

async function handlePost(req: NextRequest, dependencies: TradingSessionDependencies) {
  if (!privateAgentTransportAllowed("session", process.env, dependencies.fetchImpl)) {
    return applyNoStore(NextResponse.json(
      { error: "private_execution_session_disabled" },
      { status: 503 },
    ));
  }
  if (!sameOrigin(req)) {
    return applyNoStore(NextResponse.json(
      { error: "cross_site_trading_session_rejected" },
      { status: 403 },
    ));
  }
  ...
  const webSession = await dependencies.fetchSessionUserImpl(webSessionToken).catch(() => null);
  if (!webSession?.ok) {
    return applyNoStore(NextResponse.json({ error: "web_session_invalid" }, { status: 401 }));
  }
  const subjectScope = terminalLiveExecutionSubjectScope(webSession.user.id);
  if (!subjectScope) {
    return applyNoStore(NextResponse.json({ error: "web_session_subject_invalid" }, { status: 403 }));
  }
  ...
  const upstream = await dependencies.fetchImpl(buildV1Url(GHOLA_EXECUTION_API_BASE, BRIDGE_PATH), {
    method: "POST",
    headers: {
...
export async function handleTradingSessionPost(req: NextRequest) {
