...
export async function sealHyperliquidVaultFromBody(
  ...
  const signerBinding = await verifyHyperliquidCredentialBinding({
    value: value.credential_binding,
    account_commitment: account.account_commitment,
  ...
  const stored = await putHyperliquidExecutionVault({
    ...
    vault: { ...created.vault, signer_binding: signerBinding.binding },
...
type VerifiedHyperliquidSignerBinding = NonNullable<
  PrivateHyperliquidVaultRecordV1["vault"]["signer_binding"]
>;

async function verifyHyperliquidCredentialBinding(input: {
  value: unknown;
  account_commitment: string;
  network: "mainnet" | "testnet";
}): Promise<
  | { ok: true; binding: VerifiedHyperliquidSignerBinding }
  | {
      ok: false;
      error:
        | "hyperliquid_agent_binding_required"
        | "hyperliquid_agent_binding_invalid"
        | "hyperliquid_agent_not_authorized"
        | "hyperliquid_binding_check_unavailable";
    }
> {
  const value = objectBody(input.value);
  const ownerAddress = stringValue(value.owner_address).toLowerCase();
  const agentAddress = stringValue(value.agent_address).toLowerCase();
  const signature = stringValue(value.signature) as `0x${string}`;
  if (
    value.version !== 1 ||
    value.network !== input.network ||
    !/^0x[0-9a-f]{40}$/.test(ownerAddress) ||
    !/^0x[0-9a-f]{40}$/.test(agentAddress) ||
    !/^0x[0-9a-f]{130}$/.test(signature)
  ) {
    return { ok: false, error: Object.keys(value).length === 0
      ? "hyperliquid_agent_binding_required"
      : "hyperliquid_agent_binding_invalid" };
  }
  try {
    const recovered = await recoverMessageAddress({
      message: hyperliquidApiWalletBindingMessage({
        accountCommitment: input.account_commitment,
    ...
    if (recovered.toLowerCase() !== agentAddress) {
      return { ok: false, error: "hyperliquid_agent_binding_invalid" };
    }
  } catch {
    return { ok: false, error: "hyperliquid_agent_binding_invalid" };
  }
  const venueCheck = await verifyHyperliquidAgentAtVenue({
    network: input.network,
  ...
  return {
    ...
    binding: {
      ...
      owner_address: ownerAddress,
      agent_address: agentAddress,
      binding_commitment: gholaCommitment("hyperliquid_signer_binding", {
        account_commitment: input.account_commitment,
        network: input.network,
        owner_address: ownerAddress,
        agent_address: agentAddress,
...
async function verifyHyperliquidAgentAtVenue(input: {
  network: "mainnet" | "testnet";
...
> {
  ...
  try {
    const response = await fetchWithTimeout(`${baseUrl}/info`, {
      ...
      body: JSON.stringify({ type: "extraAgents", user: input.owner_address }),
    });
...
async function reverifyStoredHyperliquidSignerBinding(
  ...
  | { ok: true; binding: VerifiedHyperliquidSignerBinding }
  | { ok: false; error: "hyperliquid_agent_binding_required" | "hyperliquid_agent_not_authorized" | "hyperliquid_binding_check_unavailable" }
> {
  const binding = vault.vault.signer_binding;
  if (!binding) return { ok: false, error: "hyperliquid_agent_binding_required" };
  const checked = await verifyHyperliquidAgentAtVenue(binding);
  return checked.ok ? { ok: true, binding } : checked;
}
...
export async function connectorVerifyNoSubmitFromBody(
  ...
  if (platformClass === "hyperliquid_style_market") {
    ...
    if (allocation?.owner_commitment === owner.owner_commitment && allocation.status === "allocated") {
      ...
      hyperliquidNetwork = allocation.allocation.network;
      connectorVault = {
        venue_id: "hyperliquid",
        execution_mode: allocation.allocation.execution_mode,
      ...
      const signerBinding = await reverifyStoredHyperliquidSignerBinding(vault);
      if (!signerBinding.ok) return { error: signerBinding.error };
      executionMode = "byo_api_key";
      hyperliquidNetwork = signerBinding.binding.network;
      hyperliquidSignerBindingVerified = true;
      connectorVault = {
        venue_id: "hyperliquid",
        execution_mode: "byo_api_key",
        vault_commitment: vault.vault_commitment,
  ...
  if (platformClass === "hyperliquid_style_market" &&
    ...
    const proof = {
      ...
      network: hyperliquidNetwork ?? (process.env.GHOLA_HYPERLIQUID_LIVE_MODE === "tiny_fill" ||
        process.env.GHOLA_HYPERLIQUID_LIVE_MODE === "full_ticket" ? "mainnet" : "testnet"),
...
export async function connectorReconcileFromBody(
  ...
  const workOrderRecord = workOrderCommitment
    ...
    : previewCommitment
      ? await getConnectorWorkOrderByPreview(previewCommitment)
    : existingResult
  ...
  if (workOrderRecord.platform_class === "hyperliquid_style_market") {
    ...
    if (allocationRecord?.owner_commitment === owner.owner_commitment && allocationRecord.status === "allocated") {
      hyperliquidManagedAllocation = allocationRecord.allocation;
    } else if (vaultRecord?.owner_commitment === owner.owner_commitment && vaultRecord.status === "sealed") {
      const signerBinding = await reverifyStoredHyperliquidSignerBinding(vaultRecord);
      if (!signerBinding.ok) return { error: signerBinding.error };
      hyperliquidExecutionVault = vaultRecord.vault;
    } else {
      return { error: "hyperliquid_execution_vault_not_ready" as const };
    }
  }
...
> {
  ...
  if (
    ...
    if (!hyperliquidVault || hyperliquidVault.status !== "sealed") {
      return { error: "hyperliquid_agent_binding_required" };
    }
  ...
  if (["submitting", "ambiguous", "submitted"].includes(workOrderRecord.status)) {
  ...
  try {
    ...
    if (!submitted.ok) {
      ...
      if (balanceReservation && !ambiguous) {
