...
function request(body: unknown, apiKey = "sk_treasury") {
  return new Request("https://ghola.test/v1/treasury-intents/reconcile", {
    method: "POST",
    headers: {
...
function executeRequest() {
  return new Request("https://ghola.test/v1/treasury-intents/execute", {
    ...
    body: JSON.stringify({
      version: 1,
      intent_id: "intent_treasury_reconcile",
      owner_did: OWNER,
...
describe("treasury intent reconciliation route", () => {
  afterEach(async () => {
  ...
  it("requires a valid treasury agent API key", async () => {
    ...
    const res = await POST(request({ version: 1, intent_id: "intent_treasury_reconcile" }, ""));
  ...
  it("reconciles submitted partner refs and persists submitted state", async () => {
    readyEnv();
    ...
    const res = await POST(
      request({ version: 1, intent_id: "intent_treasury_reconcile" }),
    );
    const body = await res.json();
    ...
    expect(
      body.reconciliations.every(
        (item: { reconciliation_state: string }) =>
          item.reconciliation_state === "submitted",
      ),
    ).toBe(true);
  });
});
