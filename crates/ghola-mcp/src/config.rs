...
impl ThumperConfig {
    ...
    pub fn load() -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let mut config = if config_path.exists() {
            ...
            let relay_url = ghola_assistant_types::env_compat("GHOLA_RELAY_URL", "THUMPER_RELAY_URL")
                .unwrap_or_else(|_| "ws://localhost:8080/ws".to_string());
            let mcp_pubkey = ghola_assistant_types::env_compat("GHOLA_MCP_PUBKEY", "THUMPER_MCP_PUBKEY")
                .unwrap_or_else(|_| "not_configured".to_string());
            let device_pubkey = ghola_assistant_types::env_compat("GHOLA_DEVICE_PUBKEY", "THUMPER_DEVICE_PUBKEY")
                .unwrap_or_else(|_| "not_configured".to_string());
            let timeout_secs = ghola_assistant_types::env_compat("GHOLA_TIMEOUT", "THUMPER_TIMEOUT")
                .ok()
