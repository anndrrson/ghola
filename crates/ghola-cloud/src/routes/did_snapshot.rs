...
//! checked against the `GHOLA_CLOUD_RELAY_API_KEY` env var. Only the
...
/// against `GHOLA_CLOUD_RELAY_API_KEY`.
pub async fn get_did_set(
    State(state): State<AppState>,
    headers: HeaderMap,
) -> Result<Json<DidSetSnapshot>, CloudError> {
    // -- API-key auth --------------------------------------------------
    let expected = ghola_assistant_types::env_compat("GHOLA_CLOUD_RELAY_API_KEY", "THUMPER_CLOUD_RELAY_API_KEY").map_err(|_| {
        tracing::error!("GHOLA_CLOUD_RELAY_API_KEY unset; refusing /v1/did-set");
