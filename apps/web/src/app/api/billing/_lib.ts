...
export async function proxyBillingRequest(
  ...
  if (!bearer) return NextResponse.json({ error: "sign in required" }, { status: 401, headers: NO_STORE_HEADERS });
  if (method === "POST" && req.cookies.get(SESSION_COOKIE_NAME)?.value && !sameOrigin(req)) {
    return NextResponse.json({ error: "cross-site request rejected" }, { status: 403, headers: NO_STORE_HEADERS });
  }
  ...
  if (!upstream) return NextResponse.json({ error: "billing unavailable" }, { status: 503, headers: NO_STORE_HEADERS });
