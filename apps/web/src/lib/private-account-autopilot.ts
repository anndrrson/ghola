...
function strategyValue(value: unknown): AutopilotSession["strategy"] | null {
  ...
  const decisionModel = stringValue(raw.decision_model) === "ai_direct_order_v1"
    ? "ai_direct_order_v1"
    : "rules_plus_ai_score";
  const proposalOnly = raw.model_role === "proposal_only" || raw.decision_contract === "structured_proposal_v2";
  const aiCanExecuteDirectly = !proposalOnly && (raw.ai_can_execute_directly === true || decisionModel === "ai_direct_order_v1");
  return {
    version: 1,
    strategy_id: "bounded_intent_executor_v1",
    decision_model: decisionModel,
    executable_order_source: proposalOnly
      ? "deterministic_cost_router_after_typed_model_proposal"
      : aiCanExecuteDirectly
        ? "ai_structured_decision_validated_by_policy"
        : "deterministic_bounded_intent_executor",
    ai_can_execute_directly: aiCanExecuteDirectly,
    decision_contract: proposalOnly ? "structured_proposal_v2" : "deterministic_proposal_v1",
    model_role: proposalOnly ? "proposal_only" : "score_only",
    deterministic_router: proposalOnly,
  };
}
...
// steers the model proposal with the rest. "auto" / absent lets the model propose direction.
